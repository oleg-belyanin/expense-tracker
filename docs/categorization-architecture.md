...
12. Выбор категории и порядок dropdown по месту считаются раздельно. Неоднозначное место
    не назначает категорию и не усредняется как слово названия, но ранжирует пикер.
    Исключение выбора: пустое имя, сырая ничья места 50/50 и живой категорийный транзит
    между этими двумя категориями — побеждает цель последнего транзита (дух F-04).
13. На форме расхода пустой фокус названия и места показывает 3 недавних значения;
    после первой буквы — до 8 prefix-подсказок. Фильтр списка мест этот лимит не меняет.
...
Место без достаточной поддержки или с близким к равномерному распределением **не назначает**
категорию. Порог `locationEligible` считается по сырым счётчикам, без сглаживания Лапласа:

```text
support(location) >= MIN_SEED_SUPPORT
maxShare(location) >= MIN_SEED_PROBABILITY
```
...
Вектор при этом всё равно строится для dropdown: пикер показывает категории по убыванию
score, даже если автоподстановка — «Прочее».

Место не добавляется в средний вектор слов названия как ещё один keyword и не получает
keyword-транзит. Транзитные поправки остаются на словах имени.

#### Ничья 50/50 и живой категорийный транзит

Если одновременно:

- вектора имени нет (пустое или неизвестное название);
- по месту ровно две активные категории с равным сырым счётчиком (доля 50/50);
- есть открытый `category_transition` с хотя бы одним активным словом между этими
  двумя категориями;

то выбранная категория — цель последнего такого транзита (`created_at DESC`).
Чужой транзит (другая пара категорий) ничью не разводит. Если вектор имени есть,
правило не применяется: название важнее смешанного места.

Пример: место «Гашека», расходы «Здоровье» и «Стоматология» по одному разу,
живой транзит Здоровье → Стоматология. Автоподстановка — Стоматология; в пикере
обе категории сверху. Без живого транзита автоподстановка остаётся «Прочее»,
ранжирование пикера то же.
...
Для **выбора** в `combine` входит только eligible-место. Для **dropdown**:

- имя есть и место eligible — тот же `combine`;
- имя есть и место не eligible — только `Vname` (неоднозначный магазин не размывает название);
- имени нет — `Vlocation`, даже если место не eligible.
...
9. Построить `Vlocation`, если по месту есть счётчики; запомнить `locationEligible`.
10. Для выбора: `combine(Vname, Vlocation)`, если место eligible, иначе только `Vname`.
11. Если `Vname` нет, место 50/50 и есть живой транзит между этими двумя категориями —
    выбрать цель последнего транзита.
12. Иначе `stableArgMax` выборного вектора либо fallback «Прочее».
13. Для dropdown: `combine`, только `Vname` или `Vlocation` по правилам §9.7.
    Fallback не обнуляет блок «Подходят лучше», если ранжирующий вектор есть.
14. Удалить из кандидатов архивные категории.
15. Отсортировать dropdown по убыванию score, при равенстве `category_id ASC`.
16. Вернуть выбранную категорию, dropdown и объяснение результата.
...
locationVector = vectorFor(input.location)          // даже если не eligible
locationEligible = locationEligible(raw location counts)
selectionVector = combine(nameVector, locationVector.takeIf { locationEligible })
rankingVector =
    if nameVector != null && locationEligible: combine(nameVector, locationVector)
    else if nameVector != null: nameVector
    else: locationVector

if nameVector is absent
   and evenTwoWaySplit(location counts)
   and liveCategoryTransition between those two:
    selected = latestTransition.toCategory
else if selectionVector is absent:
    selected = OTHER
else:
    selected = stableArgMax(selectionVector)

return selected, sortedCandidates(rankingVector)
```

Точный выбор и порядок dropdown вычисляются отдельно. Даже при наличии точного правила
вероятностный вектор рассчитывается для показа альтернатив. Fallback «Прочее» не скрывает
ранжированный список, если `rankingVector` есть.
...
Если задано только место с неоднозначной статистикой (например «Гашека»:
Здоровье 1, Стоматология 1), автоподстановка — «Прочее», пока нет живого транзита
между этими категориями. Пикер всё равно ставит обе категории первыми.
...
На форме расхода пустой фокус названия и места показывает **3** самых недавних
значения (`EMPTY_FOCUS_LIMIT`): это ярлыки повтора, а не каталог. После первой
буквы — обычный prefix, до **8** строк (`DEFAULT_LIMIT`). Выбор подсказки
подставляет полное имя и пересчитывает категорию уже по завершённому слову.

Фильтр списка расходов по месту этот лимит не меняет: пустой запрос по-прежнему
даёт до 8 недавних мест.
...
или
Прочее, пикер: Здоровье и Стоматология сверху
  место «Гашека» неоднозначно (1:1), живого транзита нет
или
Стоматология — ничья места 50/50:
  живой транзит Здоровье → Стоматология
```
...
- неоднозначное место не назначает категорию, но ранжирует dropdown;
- неоднозначное место не размывает `Vname`;
- ничья места 50/50 при живом транзите выбирает цель последнего транзита;
- чужой транзит ничью места не разводит;
- вектор имени блокирует правило ничьей места.
...
- пустой фокус формы расхода: 3 недавних названия и места;
- после первой буквы на форме: до 8 prefix-подсказок;
- фильтр списка мест сохраняет лимит 8 на пустом запросе;
...
| F-03 | Seed-словарь, векторы слов и мест, раздельные выбор и dropdown, fallback |
...
Лимиты автодополнения формы не входят в seed-конфиг: `EMPTY_FOCUS_LIMIT = 3`
и `DEFAULT_LIMIT = 8` заданы в `SuggestExpenseNamesUseCase` /
`SuggestLocationsUseCase`.

Seed-параметры не разбрасываются по UI: они хранятся в одной конфигурации,
...
4. одного конфигурируемого объединения с вектором места, раздельного выбора
   и dropdown, ничьей 50/50 при живом категорийном транзите;
...
11. Room FTS5 для поиска и автодополнения мест и наименований
    (на форме: 3 недавних на пустом фокусе, 8 после первой буквы);
