...
Схема версии 1 экспортируется в `core/database/schemas/`. Список миграций —
`AppMigrations.all`, подключается через `addMigrations`. `fallbackToDestructiveMigration`
не используется: неизвестная версия падает, а не стирает данные пользователя.
Следующая версия добавляет `Migration(1, 2)`, поднимает `@Database(version)` и
коммитит `2.json`.
