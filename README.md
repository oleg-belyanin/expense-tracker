...
3. seed exact rule;
4. средний вектор слов названия, затем смесь с **eligible**-вектором места;
5. fallback «Прочее».

Неоднозначное место не назначает категорию и не размывает название, но ранжирует
пикер. Исключение: пустое имя, сырая ничья места 50/50 и живой категорийный транзит
между этими двумя категориями — выбирается цель последнего транзита.
...
показываются подсказки названия из истории и словаря. На форме пустой фокус
названия и места даёт 3 недавних значения; после первой буквы — до 8.
